  ...
  const lineRenderer = ({ rows, stylesheet, useInlineStyles }) => {
    ...
    return rows.map((node, i) => {
      ...
      if (
        ...
        if (
          addedLines.some(
            ([addedStart, addedEnd]) => lineNumber >= addedStart && lineNumber <= addedEnd
          )
        ) {
          ...
          removedLines.some(
            ([removedStart, removedEnd]) => lineNumber >= removedStart && lineNumber <= removedEnd
          )
        ) {
        ...
        return (
          <span style={lineStyle} key={i}>
            <Line key={i} lineNumber={lineNumber - removedCount} showLineNumbers={showLineNumbers}>
              {createElement({
                node,
                stylesheet,
                useInlineStyles,
        ...
        <>
          {fileName || title ? (
            <div className="align-center flex gap-3 px-3 py-1">
              ...
              {title && <span className="flex items-center text-muted-foreground">{title}</span>}
            </div>
          ) : null}
          {rawHTML ? (
            <pre style={preStyles}>
