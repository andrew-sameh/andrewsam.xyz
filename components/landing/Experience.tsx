    ...
    return experiences.map((experience) => ({
      ...
      children: (
        <>
          ...
          <TimelineItemDateRange startDate={new Date(experience.startDate)} endDate={experience.endDate ? new Date(experience.endDate) : undefined} />
    ...
    <div className="my-10 w-full">
      ...
      <div className="mt-5">
        ...
        >
            ...
            {experienceData.map((company) => (
              ...
              >
                <div className="flex w-full items-center justify-between">
                  ...
                  <span className={`mx-1 inline-block h-3 w-3 rounded-full  ${company.active ? "bg-green-300" : ""}`} />
                </div>
              </TabsTrigger>
            ))}
          </TabsList>
          ...
          />
          {experienceData.map((company) => (
            <TabsContent key={company.name} value={company.name} className="mt-4 md:mt-0 w-full">
              <Card key={`card-${company.name}`} className="border-none shadow-sm outline-none ring-0">
                ...
                <CardContent>
                  <Timeline items={createTimelineItems(company.items)} activeItem={0} mode='manual' />
                </CardContent>
              </Card>
            </TabsContent>
          ))}
